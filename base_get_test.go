...
func TestGetRequestHook(t *testing.T) {
	...
	resp, _ := Get("http://httpbin.org/get",
		&RqOptions{BeforeRequest: addHelloWorld})
...
func TestGetNoOptionsCustomClient(t *testing.T) {
	resp, _ := Get("http://httpbin.org/get",
		&RqOptions{HTTPClient: http.DefaultClient})
	verifyOkResponse(resp, t)
}

func TestGetCustomTLSHandshakeTimeout(t *testing.T) {
	ro := &RqOptions{TLSHandshakeTimeout: 10 * time.Millisecond}
...
func TestGetCustomDialTimeout(t *testing.T) {
	ro := &RqOptions{DialTimeout: time.Nanosecond}
...
func TestGetSession(t *testing.T) {
	...
	resp, err = session.Get("http://httpbin.org/cookies/set", &RqOptions{Params: map[string]string{"two": "three"}})
	...
	resp, err = session.Get("http://httpbin.org/cookies/set", &RqOptions{Params: map[string]string{"three": "four"}})
...
func TestGetInvalidURL(t *testing.T) {
	resp, err := Get("%../dir/", &RqOptions{Params: map[string]string{"1": "2"}})
...
func TestGetInvalidSSLCertNoVerify(t *testing.T) {
	ro := &RqOptions{InsecureSkipVerify: true}
...
func TestGetInvalidSSLCertNoCompression(t *testing.T) {
	ro := &RqOptions{UserAgent: "LeviBot 0.1", DisableCompression: true}
...
func TestGetInvalidSSLCertWithCompression(t *testing.T) {
	ro := &RqOptions{UserAgent: "LeviBot 0.1", DisableCompression: false}
	resp, err := Get("https://self-signed.badssl.com/", ro)
...
func TestUnlimitedRedirects(t *testing.T) {
	...
	resp, err := Get(srv.URL+"/bar", &RqOptions{Headers: map[string]string{"X-Custom": "1"}})
...
func TestAuthStripOnRedirect(t *testing.T) {
	...
	resp, err := Get(srv.URL+"/test", &RqOptions{
		Auth:    []string{"one ", "two"},
...
func TestNoRedirect(t *testing.T) {
	...
	_, err := Get(srv.URL+"/3tester/", &RqOptions{
		HTTPClient: client,
	})
...
func TestGetCustomRequestTimeout(t *testing.T) {
	ro := &RqOptions{RequestTimeout: 2 * time.Nanosecond}
	if _, err := Get("http://httpbin.org", ro); err == nil {
		t.Error("unexpected: successful connection")
	}
}
